 ...
 */
PriorityQueue<EncodingTreeNode*> huffmanTreeForHelper(const string& str) {
    Map<char, int> orderString;
    PriorityQueue<EncodingTreeNode*> huffmanTree;

    for (char ch : str) {
        if (orderString.containsKey(ch)) {
            orderString.put(ch, orderString.get(ch) + 1);
        } else {
            orderString.put(ch, 1);
        }
    }
    if (orderString.size() == 1) {
        error("only one unique char");
    }
    for (char ch : orderString) {
        EncodingTreeNode* node = new EncodingTreeNode;
        node->ch = ch;
        huffmanTree.enqueue(node, orderString.get(ch));
    }
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    int priority0;
    int priority1;
    EncodingTreeNode* node0;
    EncodingTreeNode* node1;
    PriorityQueue<EncodingTreeNode*> huffmanTree = huffmanTreeForHelper(str);

    while (huffmanTree.size() > 1) {
        priority0 = huffmanTree.peekPriority();
        node0 = huffmanTree.dequeue();
        priority1 = huffmanTree.peekPriority();
        node1 = huffmanTree.dequeue();
        EncodingTreeNode* node = new EncodingTreeNode;
        node->zero = node0;
        node->one  = node1;
        huffmanTree.enqueue(node, priority0 + priority1);
    }
 ...
 */
void encodeTextHelper(EncodingTreeNode* node, Vector<Bit>& vec, Map<char, Vector<Bit>>& dict) {
    if (!(node->zero || node->one)) {
        dict.put(node->ch, vec);
    } else {
        vec.add(0);
        encodeTextHelper(node->zero, vec, dict);
        vec.remove(vec.size() - 1);
        vec.add(1);
        encodeTextHelper(node->one, vec, dict);
        vec.remove(vec.size() - 1);
    }
}

Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    Map<char, Vector<Bit>> dict;
    Vector<Bit> vec;
    Queue<Bit> result;
    encodeTextHelper(tree, vec, dict);
 ...
 * or bits in them, etc.
 */
void decodeTreeHelper(EncodingTreeNode*& node, Queue<Bit>& bits, Queue<char>& leaves) {
    if (!bits.isEmpty()) {
        Bit bit = bits.dequeue();
        node = new EncodingTreeNode;
        if (bit == 1) {
            EncodingTreeNode* node0;
            decodeTreeHelper(node0, bits, leaves);
            node->zero = node0;
            EncodingTreeNode* node1;
            decodeTreeHelper(node1, bits, leaves);
            node->one = node1;
        } else {
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    decodeTreeHelper(tree, bits, leaves);
    return tree;
}
