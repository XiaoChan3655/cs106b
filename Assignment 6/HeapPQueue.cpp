...
void HeapPQueue::enqueue(const DataPoint& data) {
    if (logicalSize == allocatedSize - 1) {
        DataPoint* tmp = elems;
        elems = new DataPoint[2 * allocatedSize];
    ...
    int n = logicalSize;
    while (n != 1) {
        if (elems[n/2].weight > elems[n].weight) {
            DataPoint tmp = elems[n/2];
            elems[n/2] = elems[n];
            elems[n] = tmp;
            n = n / 2;
        } else {
...
DataPoint HeapPQueue::peek() const {
    if (isEmpty()) {
        error("there is no element left");
    } else {
        return elems[1];
    }
}
...
DataPoint HeapPQueue::dequeue() {
    if (logicalSize == 0) {
        ...
        while (2*n <= logicalSize) {
            if ((2*n == logicalSize) && (elems[2*n].weight < elems[n].weight)) {
                DataPoint tmp = elems[2*n];
                elems[2*n] = elems[n];
                elems[n] = tmp;
                break;
            } else if ((elems[2*n].weight < elems[n].weight) ||
                       (elems[2*n+1].weight < elems[n].weight)) {
