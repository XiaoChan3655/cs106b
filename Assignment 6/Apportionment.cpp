...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    HeapPQueue pq;
    Map<string, int> stateSeats;
    for (string state : populations) {
        stateSeats.put(state, 1);
        pq.enqueue({state, static_cast<double>(-populations.get(state)) / sqrt(2)});
        numSeats -= 1;
    }
    if (numSeats < 0) {
        error("each state has at least one seat");
    }
    for (int i = 0; i < numSeats; i++) {
        DataPoint tmp = pq.dequeue();
        stateSeats[tmp.name] += 1;
        tmp.weight = (-populations.get(tmp.name)) / sqrt((stateSeats[tmp.name] + 1) * stateSeats[tmp.name]);
        pq.enqueue(tmp);
    }
    return stateSeats;
}
