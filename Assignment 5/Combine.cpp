...
Vector<DataPoint> merge(const Vector<DataPoint>& sequences1, const Vector<DataPoint>& sequences2) {

    int i = 0;
    int j = 0;
    Vector<DataPoint> result;

    while ((i < sequences1.size()) && (j < sequences2.size())) {
        if (sequences1[i].weight > sequences2[j].weight) {
            result += sequences2[j];
            j++;
        } else {
            result += sequences1[i];
    ...
    if (i == sequences1.size()) {
        result += sequences2.subList(j);
    } else {
        result += sequences1.subList(i);
    }
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if (sequences.isEmpty()) {
        ...
        return merge(combine(sequences.subList(0, sequences.size() / 2)), combine(sequences.subList(sequences.size() / 2)));
    }
}
