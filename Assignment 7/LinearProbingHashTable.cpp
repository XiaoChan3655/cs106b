...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    myHash = hashFn;
    numSlots = myHash.numSlots();
    elems = new Slot[numSlots];
    for (int i = 0; i < numSlots; i++) {
        elems[i] = {"none", SlotType::EMPTY};
    }
}
...
bool LinearProbingHashTable::insert(const string& elem) {
    if (logicalSize == numSlots) {
        return false;
    }

    int start_index = myHash(elem);
    int first_tomb  = 0;
    bool find_tomb  = false;

    for (int i = 0; i < numSlots; i++) {
        int index = (start_index + i) % numSlots;
        if (elems[index].type == SlotType::EMPTY) {
            if (find_tomb) {
                elems[first_tomb] = {elem, SlotType::FILLED};
            } else {
                elems[index] = {elem, SlotType::FILLED};
            }
            logicalSize += 1;
            return true;
        } else if ((elems[index].type == SlotType::TOMBSTONE) && !find_tomb) {
            find_tomb  = true;
            first_tomb = index;
...
void LinearProbingHashTable::printDebugInfo() const {
    for (int i = 0; i < numSlots; i++) {
        if (elems[i].type == SlotType::FILLED) {
            cout<<"slot"<<i<<":value "<<elems[i]<<" type"<<elems[i].type;
            cout<<endl;
        }
    }
}
