...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    int start_index = myHash(elem);
    int distance    = 0;
    string local_elem = elem;

    for (int offset = 0; offset < allocatedSize; offset++) {
        int index = (start_index + offset) % allocatedSize;
        if (elems[index].distance == EMPTY_SLOT) {
            elems[index].value    = local_elem;
            elems[index].distance = distance;
            logicalSize++;
            return true;
        } else if (distance > elems[index].distance) {
            Slot tmp     = {local_elem, distance};
            local_elem   = elems[index].value;
            distance     = elems[index].distance;
            elems[index] = tmp;
        } else if (elems[index].value == local_elem) {
            return false;
        }
        distance++;
    }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for (int offset = 0; offset < allocatedSize; offset++) {
        int index = (start_index + offset) % allocatedSize;
        if ((elems[index].distance == EMPTY_SLOT) || (distance > elems[index].distance)) {
            return false;
        } else if (elems[index].value == elem) {
            return true;
        }
        distance++;
    }
...
void RobinHoodHashTable::backShift(int start) {
    int index;
    int next_index;

    for (int offset = 0; offset < allocatedSize; offset++) {
        index = (start + offset) % allocatedSize;
        next_index = (start + offset + 1) % allocatedSize;
        if ((elems[next_index].distance == EMPTY_SLOT) || (elems[next_index].distance == 0)) {
            elems[index].distance = EMPTY_SLOT;
            return;
        } else {
            elems[index] = elems[next_index];
            elems[index].distance -= 1;
            elems[next_index].distance = EMPTY_SLOT;
        }
    }
}
