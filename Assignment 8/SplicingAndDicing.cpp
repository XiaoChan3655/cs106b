...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    Nucleotide* dna_ptr = dna;
    Nucleotide* tar_ptr;
    Nucleotide* mat_ptr = nullptr;
    ...
    while (dna_ptr) {
        ...
        mat_ptr = dna_ptr;
        while (tar_ptr) {
            if ((tar_ptr->value == dna_ptr->value) && !tar_ptr->next) {
                return mat_ptr;
            } else if (!dna_ptr->next) {
                return nullptr;
            } else if (tar_ptr->value != dna_ptr->value) {
                break;
            }
            tar_ptr = tar_ptr->next;
            dna_ptr = dna_ptr->next;
        }
        dna_ptr = mat_ptr->next;
    }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    Nucleotide* tar_ptr = target;
    Nucleotide* tmp;
    Nucleotide* prev_ptr;
    Nucleotide* first = findFirst(dna, target);
    Nucleotide* dna_ptr = first;
    bool first_hit;
    ...
    if (first) {
        first_hit = !first->prev;
        if (!first_hit) {
            prev_ptr = dna_ptr->prev;
        }
        while (tar_ptr) {
            tar_ptr = tar_ptr->next;
            tmp = dna_ptr;
        ...
        if (dna_ptr) {
            if (first_hit) {
                dna = dna_ptr;
                dna_ptr->prev = nullptr;
            } else {
                prev_ptr->next = dna_ptr;
                dna_ptr->prev = prev_ptr;
            }
        } else {
            if (first_hit) {
                dna = dna_ptr;
...
STUDENT_TEST("test deleteNucleotides function.") {
...
STUDENT_TEST("test fromDNA function") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'A';
    dna->prev = nullptr;
