...
void allEmphasesOfHelper(string str, const Vector<string>& seg, Set<string>& results) {
    if (seg.isEmpty()) {
        results.add(str);
    } else if (isalpha(seg[0][0])) {
        allEmphasesOfHelper(str + toUpperCase(seg[0]), seg.subList(1), results);
        allEmphasesOfHelper(str + toLowerCase(seg[0]), seg.subList(1), results);
    } else {
        allEmphasesOfHelper(str + seg[0], seg.subList(1), results);
    }
}

Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> seg = tokenize(sentence);
    Set<string> results;
    string str = "";
    allEmphasesOfHelper(str, seg, results);
    return results;
}
