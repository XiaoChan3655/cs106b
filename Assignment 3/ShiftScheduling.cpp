...
void helper(Set<Shift>& shifts, int maxHours, Set<Shift> curSchedule, Set<Shift>& bestSchedule, int curValue, int& maxValue) {
    if (shifts.isEmpty() || maxHours == 0) {
        ...
        Shift curShift = shifts.first();
        shifts = shifts - curShift;
        bool hasConflict = false;
        for (Shift sht : curSchedule) {
            if (overlapsWith(curShift, sht)) {
                hasConflict = true;
            }
        }
        if (!hasConflict && maxHours - lengthOf(curShift) >= 0) {
            //curSchedule.add(curShift);
            helper(shifts, maxHours - lengthOf(curShift), curSchedule + curShift, bestSchedule, curValue + valueOf(curShift), maxValue);
        }
        helper(shifts, maxHours, curSchedule, bestSchedule, curValue, maxValue);
        shifts.add(curShift);
    }
}

Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    Set<Shift> curSchedule;
    Set<Shift> bestSchedule;
    if (maxHours < 0) {
        error("maxHours should not be less than zero");
    }
    int maxValue = 0;
    int curValue = 0;
    Set<Shift> shiftsCopy = shifts;
    helper(shiftsCopy, maxHours, curSchedule, bestSchedule, curValue, maxValue);
    return bestSchedule;
}
