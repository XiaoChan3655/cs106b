...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork, int numCities,
                                  Set<string>& supplyLocations, const Set<string>& uncoveredLocations) {
    ...
    string city1 = uncoveredLocations.first();
    Set<string> city1Neighbor = roadNetwork[city1];
    supplyLocations.add(city1);

    if (canBeMadeDisasterReadyHelper(roadNetwork, numCities - 1, supplyLocations, uncoveredLocations - city1 - city1Neighbor)) {
        return true;
    } else {
        supplyLocations -= city1;
        for (const string& city2 : city1Neighbor) {
            supplyLocations += city2;
            Set<string> city2Neighbor = roadNetwork[city2];
            if (canBeMadeDisasterReadyHelper(roadNetwork, numCities-1, supplyLocations, uncoveredLocations - city2 - city2Neighbor)) {
                return true;
            }
            supplyLocations -= city2;
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if (numCities < 0) {
        ...
        for (string cty : roadNetwork) {
            uncoveredLocations.add(cty);
        }
        return canBeMadeDisasterReadyHelper(roadNetworkCopy, numCities, supplyLocations, uncoveredLocations);
    }
}
...
STUDENT_TEST("TEST") {
    Map<string, Set<string>> map = makeSymmetric({
        { {"A", {"C"}}, {"B", {"C"}}, {"C", {"A", "B", "E"}}, {"D", {"E"}}, {"E", {"C", "D", "F"}}, {"F", {"E"}}}
    });
    Set<string> locations;
    canBeMadeDisasterReady(map, 2, locations);
}
...
PROVIDED_TEST("Can solve ethene example, regardless of ordering.") {
    ...
    do {
         ...
         */
        Set<string> chosen;
        if (!canBeMadeDisasterReady(map, 2, chosen)) {
            std::cout << map << endl;
        }
