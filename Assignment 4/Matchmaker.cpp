...
bool hasPerfectMatchingHelper(Map<string, Set<string>>& possibleLinks, Set<Pair>& matching, Set<string>& aleradyMatch) {
    if (possibleLinks.isEmpty()) {
        return true;
    } else {
        string person = possibleLinks.firstKey();
        Set<string> personRelationship = possibleLinks.get(person);
        possibleLinks.remove(person);
        aleradyMatch.add(person);
        for (string p : personRelationship) {
            Pair personMatch(person, p);
            if (!aleradyMatch.contains(p)) {
                matching.add(personMatch);
                aleradyMatch.add(p);
                Set<string> pRelationship = possibleLinks.get(p);
                possibleLinks.remove(p);
                if (hasPerfectMatchingHelper(possibleLinks, matching, aleradyMatch)) {
                    return true;
                }
                matching.remove(personMatch);
                aleradyMatch.remove(p);
                possibleLinks.put(p, pRelationship);
            }
        }
        possibleLinks.put(person, personRelationship);
        aleradyMatch.remove(person);
        return false;
    }
}
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    Map<string, Set<string>> possibleLinksCopy = possibleLinks;
    Set<string> aleradyMatch;
    return hasPerfectMatchingHelper(possibleLinksCopy, matching, aleradyMatch);
}

void maximumWeightMatchingHelper(Map<string, Map<string, int>>& possibleLinks, int& maxWeight, int& curWeight,
                                 Set<Pair>& bestMatch, Set<Pair>& curMatch, Set<string>& aleradyMatch) {
    if (possibleLinks.isEmpty()) {
        ...
        string person = possibleLinks.firstKey();
        Map<string, int> personRelationship = possibleLinks.get(person);
        possibleLinks.remove(person);
        aleradyMatch.add(person);
        maximumWeightMatchingHelper(possibleLinks, maxWeight, curWeight, bestMatch, curMatch, aleradyMatch);
        possibleLinks.put(person, personRelationship);
        aleradyMatch.remove(person);
        for (auto p : personRelationship) {
            Pair personMatch(person, p);
            if (!aleradyMatch.contains(p) && !aleradyMatch.contains(person)) {
                curMatch.add(personMatch);
                aleradyMatch.add(p);
                aleradyMatch.add(person);
                auto pRelationship = possibleLinks.get(p);
                int weight = possibleLinks[p][person];
                curWeight += weight;
                possibleLinks.remove(p);
                possibleLinks.remove(person);
                maximumWeightMatchingHelper(possibleLinks, maxWeight, curWeight, bestMatch, curMatch, aleradyMatch);
                curWeight = curWeight - weight;
                curMatch.remove(personMatch);
                aleradyMatch.remove(p);
                aleradyMatch.remove(person);
                possibleLinks.put(p, pRelationship);
                possibleLinks.put(person, personRelationship);
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    auto possibleLinksCopy = possibleLinks;
    int maxWeight = 0;
    int curWeight = 0;
    Set<Pair> bestMatch;
    Set<Pair> curMatch;
    Set<string> aleradyMatch;
    maximumWeightMatchingHelper(possibleLinksCopy, maxWeight, curWeight, bestMatch, curMatch, aleradyMatch);
    return bestMatch;
}
...
STUDENT_TEST("MY TEST") {
    ...
    Set<Pair> matching;
    bool result = hasPerfectMatching(links, matching);
}
...
PROVIDED_TEST("hasPerfectMatching works on a line of six people.") {
    ...
    do {
        ...
        Set<Pair> matching;
        EXPECT(hasPerfectMatching(links, matching));
        if (!isPerfectMatching(links, matching)) {
            std::cout << links << endl;
        }
        EXPECT(isPerfectMatching(links, matching));
