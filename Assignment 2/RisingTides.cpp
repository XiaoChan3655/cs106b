...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    int r = terrain.numRows();
    int c = terrain.numCols();
    Grid<bool> result(r, c, false);
    Queue<GridLocation> waterPoints;
    for (GridLocation src : sources) {
        if (terrain.get(src) < height) {
            result.set(src, true);
            waterPoints.enqueue(src);
        }
    }
    while (!waterPoints.isEmpty()) {
        GridLocation point = waterPoints.dequeue();
        GridLocation newPoint;
        r = point.row;
        c = point.col;
        if (result.inBounds(r+1, c) && (terrain.get(r+1, c) < height) && (result.get(r+1, c) == false)) {
            newPoint.row = r + 1;
            newPoint.col = c;
            result.set(newPoint, true);
            waterPoints.enqueue(newPoint);
        }
        if (result.inBounds(r-1, c) && (terrain.get(r-1, c) < height && (result.get(r-1, c) == false))) {
            newPoint.row = r - 1;
            newPoint.col = c;
        ...
        if (result.inBounds(r, c+1) && (terrain.get(r, c+1) < height) && (result.get(r, c+1) == false)) {
            newPoint.row = r;
            newPoint.col = c + 1;
            result.set(newPoint, true);
            waterPoints.enqueue(newPoint);
