...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if (kGramLength <= 0) {
        error("kGramLenth less than 0 is not allowed");
    } else if (str.length() < kGramLength) {
        return results;
    } else {
        for (int i = 0; i < (str.length() - kGramLength + 1); i++) {
            results[str.substr(i, kGramLength)] += 1;
        }
    }
    return results;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    double squareSum = 0;
    for (string singleStr : input) {
        squareSum += input[singleStr] * input[singleStr];
    }
    if (squareSum == 0) {
        error("No valid map value");
    }
    for (string singleStr : input) {
        result.put( singleStr, input[singleStr] / sqrt(squareSum));
    }
    return result;
}

PriorityQueue<string> topKGramsInHelper(const Map<string, double>& source, int numToKeep) {
    PriorityQueue<string> pq;
    for (string singleStr : source) {
        if (pq.size() < numToKeep) {
            pq.enqueue(singleStr, source[singleStr]);
        } else {
            pq.enqueue(singleStr, source[singleStr]);
            pq.dequeue();
        }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if (numToKeep < 0) {
        ...
        pq = topKGramsInHelper(source, numToKeep);
        while (pq.size() > 0) {
            string str = pq.dequeue();
            results.put(str, source[str]);
        }
        return results;
    }
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()) {
        ...
        for (Corpus cps : corpora) {
            if (cosineSimilarityOf(textProfile, cps.profile) > curSimilar) {
                curSimilar = cosineSimilarityOf(textProfile, cps.profile);
                mostSimilar = cps.name;
            }
        }
        return mostSimilar;
    }
...
STUDENT_TEST("kGramsIn works when str.length() < kGramLength") {
    Map<string, double> expected = {};

    EXPECT_EQUAL(kGramsIn("str", 4), expected);
}
